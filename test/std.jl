...
@testset "StdVector" begin
    cxx_int_v = icxx"std::vector<int>{2, 4, 6, 8, 10, 12, 14};";
    ...
    @testset "StdVector getindex and setindex!" begin
        cxx_int_v2 = icxx"std::vector<int> v($cxx_int_v); v;";
        @test cxx_int_v2[3] == 8
    ...
    @testset "StdVector copy! and convert" begin
        ...
        @test begin
            dest = icxx"""std::vector<int> v(7); v;"""
            copy!(dest, jl_int_v)
            icxx"""$dest == $cxx_int_v;"""
        end
        ...
        @test begin
            dest = icxx"""std::vector<int>{41, 0, 0, 0, 42};"""
            copy!(dest, 1, jl_int_v, 4, 3)
            icxx"""$dest == std::vector<int>{41, 8, 10, 12, 42};"""
        end
