...
@inline Base.setindex!{T<:Cxx.CxxQualType}(v::StdVector{T}, val::Union{Cxx.CppValue, Cxx.CppRef}, i::Integer) =
    (@boundscheck checkbounds(v, i); icxx"($(v))[$i] = $val; void();")

@propagate_inbounds Base.setindex!{T}(v::StdVector{T}, val, i::Integer) =
    setindex!(v, convert(T, val), i)

@propagate_inbounds Base.setindex!{T<:Cxx.CxxQualType}(v::StdVector{T}, val, i::Integer) =
